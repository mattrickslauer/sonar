...
**Attendee (free).** Opens mysonar.zone on their phone (no install — it's a PWA). The radar shows their patch of the festival. They drop a photo on **Music** at the north stage ("this set is insane"); it streams to everyone nearby and starts a 24-hour countdown. As people like it, it lives longer — the genuinely great moments float; the noise expires by morning. They **ask the place** "where's the shortest beer line?" and get a summary of the last hour's drops. Each day resets clean — the radar always reflects *now*.

**Festival operator (the customer who pays).** Runs official **private channels** — "Main Stage Official", "Safety/Weather", "Lost & Found" — gated by a Lambda authorizer so only staff can post authoritative updates. They buy **sponsored permanent pins** for the medical tent, water stations, and exits (the only pins that never expire). They get the **operator view**: an anonymized congestion read of which geohash cells are hot — *the food village is overloaded, the south field is empty* — to redeploy staff in real time. Aurora DSQL holds the accounts, the sponsorship records, and the analytics rollups.
